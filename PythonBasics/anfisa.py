...
При получении этого запроса вызовите функцию what_weather() и передайте в неё значение city —
название города, в котором живёт друг. Эта функция уже есть в коде задания.

Верните результат выполнения этой функции как результат работы process_friend().

Начните с проверки: elif query == 'как погода?':

В блоке elif вызовите функцию what_weather() с аргументом city; результат вызова присвойте переменной (можно назвать её weather).
